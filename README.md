...
| **抖音热点宝** `douhot` / `douhot_hotword` | 🔐 需登录 | **没有公开 API**：页面是微前端 SPA，数据全在登录后带 `a_bogus` 签名的 XHR 里。绕不过，也不该绕（那是绕风控）。走 CDP 复用用户已登录浏览器，见下节 |
...
热点宝的价值高于抖音热榜：它给的是**创作者视角**的数据（低粉爆款、垂类榜、
热词 60 日趋势、官方活动日历），而不只是「今天大家都在搜什么」。
...
**已注册的榜**（`board` 传短名；值是一个**完整 URL**，见 `douhot.BOARDS`）：

| board | 页面 | 含哪些榜 |
|---|---|---|
| `hotspot`（默认） | `/square/hotspot?active_tab=hotspot_all` | 视频榜 + 话题榜 + 搜索榜（**一页同时打三个接口**） |
| `hotword` | `/square/trend?active_tab=hotword_all` | 内容词趋势（潜力词，带飙升指数） |

「低粉爆款」是榜单聚合页里的一个区块，数据来自同一个
`/douhot/v1/material/video_billboard` 接口（不同筛选参数）。**上一版把它当独立
榜注册成 `douhot_low_fans`，拼出的 `/low_fans` 路由并不存在** —— 而微前端对未知
路由返回 **200 的 404 页**，于是它不报 404，反而把 404 页上的微前端包注册表当成
榜单数据解析出来，最后抛「可能是未登录」。等拿到那个筛选参数再作为独立源注册。

边界（写清楚，避免误用）：

- **只读**：只 `goto` + 读响应体/可见文本。不填表、不点按钮、不下载、
  不执行页面脚本，也不读取/导出 cookie。
- **不碰用户的页面**：一律开独立 page 再导航。曾经用 `context.pages[0]`，
  会把用户正开着的那一页顶走（验收时就这么把人家看的一半导航到了 404 页）。
  读别人的浏览器不等于可以改他的页面。
- `browser.close()` 对 `connect_over_cdp` 只是**断开 CDP 连接**，不会关掉
  用户的浏览器。
- 未登录时**显式报错**（"可能是未登录"），不静默返回空列表 —— 静默空
  结果会被上层当成「今天没热点」。
- **只收 `douhot.douyin.com/douhot/v1/` 前缀下的响应**（`code == 0` 且含条目
  列表）。实测任意页——**包括 404 页**——都会拉微前端包注册表
  （`kura-cloud.zijieapi.com`）与问卷表单（`api.feelgood.cn`），二者都是
  「带 `name` 的 dict 列表」，只按形状判会照收。
- **404 要响**：`goto` 成功什么都不能证明（微前端对未知路由返回 200），
  故页面文本里出现独立成行的 `404` 就报错，并把目标 URL 打出来。
...
- 热点宝的两个源**默认不参与** `discover()` 的全源抓取（要连浏览器、要登录，
