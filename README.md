...
| **InfoQ** `rss` | ✅ | 公开 RSS；**但默认客户端 UA 会被 WAF 回 451**，换浏览器 UA 即 200 |
...
| 微博热搜第三方镜像 | ❌ | **第三方不可靠**：域名在代理下不可达；官方 ajax 接口回 `Forbidden`（要 cookie） |

**结论（修正后）**：中文热点**拿得到**，而且最贴 STEPWORK 的一条是零门槛的
—— 抖音热榜官方接口。真正的限制只有两条：微博/百度这类「热搜榜」没有免登录
的官方接口；RSSHub 这类公共聚合必须自建。
